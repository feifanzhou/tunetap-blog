...
class Post < ActiveRecord::Base
  ...
  def embed_code
    if self.player_type == 'bopfm'
      ...
      return "<iframe data-post-id='#{ self.id }' width='100%' height='#{ height }' scrolling='no' frameborder='no' src='#{ self.player_embed }'></iframe>".html_safe
  ...
  def self.posts_for_page(page = 1, posts_per_page = 10, show_deleted = false)
    p = show_deleted ? Post.all.includes(:tags, :contributor) : Post.where('is_deleted = false').includes(:tags, :contributor)
